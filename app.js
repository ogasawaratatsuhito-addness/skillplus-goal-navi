...
const RESULTS = {
  ...
  action: {
    ...
    summary: "Addnessをゴールと進捗の正本にして、セットアップしたメイン作業場で次の一手を進めます。必要な学習にはSLSも使えます。",
    action: "Addnessで進行中のゴールを開き、メイン作業場のAIエージェントと次の一手を進める。必要ならSLSのアクションマップで学ぶ。",
    done: "行動の成果と次の一手がAddnessのゴールへ戻り、同じメイン作業場から続けられる。",
    service: "Addness・メイン作業場（必要に応じてSLS）", serviceNote: "Addnessを正本に、AIエージェントと実行します。知識や手順が必要なときはSLSのアクションマップを使います。",
  ...
  stuck: {
    ...
    action: "Addnessのゴールを開き、試したこと・困っている点・欲しい結果をAIエージェントへ伝える。必要ならSLSで質問する。",
    done: "次に試すことが一つ決まり、Addnessのゴールからもう一度行動できる。",
    service: "Addness・AIエージェント（必要に応じてSLS）", serviceNote: "ゴールの文脈を保ったままAIと整理し、学習内容はスキルプラスくんや講師質問も使います。",
    cta: "Addnessでゴールを開く", url: LINKS.todo,
  ...
  review: {
    ...
    service: "Addness・メイン作業場", serviceNote: "Addnessで目標と今日の一手を見直し、同じメイン作業場のAIエージェントと次の行動へつなげます。",
